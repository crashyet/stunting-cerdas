            ...
            <div x-transition class="bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
                ...
                <form action="{{ route('admin.edukasi.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <input name="judul" type="text" class="w-full rounded-md border px-3 py-2 mt-1" required>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium">Kategori</label>
                            <select name="kategori" class="w-full mt-1 rounded-md border px-3 py-2">
                        ...
                        <div>
                            ...
                            <input name="waktu_baca" type="number" min="1"
                                class="w-full mt-1 rounded-md border px-3 py-2" required>
                        </div>
                    </div>

                    <div>
                        <label class="text-sm font-medium">Penulis</label>
                        <input name="penulis" type="text" class="w-full mt-1 rounded-md border px-3 py-2" required>
                    </div>

                    <div>
                        <label class="text-sm font-medium">Ringkasan</label>
                        <textarea name="intro" rows="2" class="w-full mt-1 rounded-md border px-3 py-2" required></textarea>
                    </div>

                    <div>
                        <label class="text-sm font-medium">Konten Artikel</label>
                        <textarea name="konten" rows="6" class="w-full mt-1 rounded-md border px-3 py-2" required></textarea>
                    </div>

                    <div>
                        <label class="text-sm font-medium">Thumbnail</label>
                        <input name="thumbnail" type="file" class="w-full mt-1 rounded-md border px-3 py-2">
                    </div>
                ...
                <tbody class="text-gray-800">
                    @foreach ($edukasis as $edukasi)
                        ...
                        <tr class="border-b hover:bg-gray-50 transition animate-[fadeUp_.6s_ease-out]">
                            <td class="py-4 px-5">{{ $edukasi->judul }}</td>
                            <td class="px-5">
                                <span
                                    class="px-3 py-1 text-xs bg-green-100 text-green-700 rounded-full">{{ $edukasi->kategori }}</span>
                            </td>
                            <td class="px-5">{{ $edukasi->penulis }}</td>
                            <td class="px-5">{{ \Carbon\Carbon::parse($edukasi->created_at)->translatedFormat('d M Y') }}
                            </td>
                            <td class="px-5 flex gap-3">
                                ...
                                <form action="{{ route('edukasi.updateStatus', $edukasi->id) }}" method="POST">
    @csrf
    @method('PATCH')

    <select name="status" onchange="this.form.submit()" class="border rounded px-2 py-1">
        <option value="draft" {{ $edukasi->status == 'draft' ? 'selected' : '' }}>Draft</option>
        <option value="publish" {{ $edukasi->status == 'publish' ? 'selected' : '' }}>Publish</option>
    </select>
</form>
