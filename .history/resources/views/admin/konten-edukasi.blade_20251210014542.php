...
@section('content')
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input name="waktu_baca" type="number" min="1" class="w-full mt-1 rounded-md border px-3 py-2" required>
        </div>
    </div>
    ...
    @foreach ($edukasis as $edukasi)
                    ...
                    <tr class="border-b hover:bg-gray-50 transition animate-[fadeUp_.6s_ease-out]">
                        ...
                        <td class="px-5">
                            <span class="px-3 py-1 text-xs bg-green-100 text-green-700 rounded-full">{{ $edukasi->kategori }}</span>
                        </td>
                        <td class="px-5">{{ $edukasi->penulis }}</td>
                        <td class="px-5">{{ \Carbon\Carbon::parse($edukasi->created_at)->translatedFormat('d M Y') }}</td>
                        <td class="px-5 flex gap-3">
                            <button class="text-gray-600 hover:text-gray-900 transition"><i
                                    class="fa-solid fa-pen"></i></button>


                            <form action="{{ route('edukasi.destroy', $edukasi->id) }}" method="POST" onsubmit="return confirm('Yakin mau hapus?')">
    @csrf
    @method('DELETE')
